...
def file_upload_to_blob(chunk_path,video_id):
    ...
    try:
        ...
        logging.info(f"Exit code: {result_back.exit_code}")
        logging.info(f"STDOUT: {result_back.stdout}")

        if(result_back.exit_code==0):
            logging.info(f"File uploaded successfully to {blob_upload_url}")

            url_for_video=f"{chunk_path}/master.m3u8"
            url_for_preview=f"{chunk_path}/thumbnail.jpg"
            
            try:
                r = celery_app_instance.send_task(
                    "db_update_task",
                    args=[video_id, url_for_video, url_for_preview],

                    queue="db_update_queue"     
                )
          
                logging.info(f"chunk_path exists: {os.path.exists(chunk_path)}")
                logging.info(f"chunk_path: {chunk_path}")

                logging.info(f"Task sent to update DB with URL: {url_for_video} with {r.id} task id")
