import os
import logging
import uuid
AZCOPY_AUTO_LOGIN_TYPE =os.getenv("AZCOPY_AUTO_LOGIN_TYPE")
AZCOPY_TENANT_ID=os.getenv("AZCOPY_TENANT_ID")
AZCOPY_SPA_APPLICATION_ID=os.getenv("AZCOPY_SPA_APPLICATION_ID")
AZCOPY_SPA_CLIENT_SECRET=os.getenv("AZCOPY_SPA_CLIENT_SECRET")
blob_upload_url=os.getenv("blob_upload_url")

from azpype.commands.copy import Copy

def download_video(blob_url,file_path):

    logging.info(f"Starting video processing==>with blob_url: {blob_url}")
    logging.info(f"Starting video processing==>with file_path: {file_path}")

    try:

        Copy(
        source=blob_url,
        destination=file_path,
        recursive=True,
       
        ).execute()

        return file_path
    except Exception as e:
        logging.error(f"Error processing with blob_url: {blob_url} ===>Error: {str(e)}")
        return
        